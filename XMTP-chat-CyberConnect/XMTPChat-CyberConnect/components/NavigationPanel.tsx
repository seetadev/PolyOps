...
import { CYBERCONNECT_ENDPOINT, GET_CONNECTIONS } from './CyberConnect'
import { Conversation } from "@xmtp/xmtp-js/dist/types/src"

// Initialize the GraphQL Client
const cyberConnectClient = new GraphQLClient(CYBERCONNECT_ENDPOINT)
...
const NavigationPanel = ({ onConnect, isFiltered }: NavigationPanelProps): JSX.Element => {
  const { walletAddress } = useXmtp()

  return (
    <div className="flex-grow flex flex-col">
      {walletAddress ? (
        <ConversationsPanel isFiltered={isFiltered} walletAddress={walletAddress} />
      ) : (
...
const ConversationsPanel = ({ isFiltered, walletAddress }: ConversationsPanelProps): JSX.Element => {
  ...
  const filtered = isFiltered ? conversations.filter((conversation: Conversation) => followings.includes(conversation.peerAddress.toLowerCase())) : conversations
