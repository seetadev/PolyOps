...
type UserMenuProps = {
  ...
  isFiltered?: boolean,
  showFiltered: React.Dispatch<React.SetStateAction<boolean>>
}
...
const UserMenu = ({ onConnect, onDisconnect, isFiltered, showFiltered }: UserMenuProps): JSX.Element => {
  ...
  const onFollowAddress = async() => {
    // Prompt to enter the address
    const address = prompt("Please enter the wallet address to follow")
    ...
    try {
      ...
      toast.success(`Success: you're following ${address}!`, {
        position: "top-right",
        autoClose: 5000,
  ...
  return (
    <div className="flex bg-n-500 items-center justify-between rounded-lg h-14 m-4 mb-5 md:mb-4 px-4 drop-shadow-xl">
      {walletAddress ? (
        ...
        <NotConnected onConnect={onConnect} isFiltered={isFiltered} showFiltered={showFiltered} />
      )}
    </div>
  )
}
