...
import { IonicPage, NavController, ToastController , Tabs, AlertController, ModalController } from 'ionic-angular';
import * as AppGeneral from 'socialcalc/AppGeneral';
import { DATA, APP_NAME, LINK , LOGO } from '../../providers/app-data';
...
export class MenuPage {
	...
	constructor(public navCtrl: NavController, 
				...
				this.request = { pdf: {done: true}, allpdf: {done: true}, save: {done: true}, sharepdf: {done: true}, logo: {done: true}};
	...
	saveAsPassword(){
	    ...
	    title: 'Save as Password Protected file',
	    inputs: [
	      {
	        name: 'filename',
	        ...
	        handler: data => {
	          if(this.validateName(data.filename)){
	          	if(this.validatePassword(data.password)){
					...
					var content = encodeURIComponent(AppGeneral.getSpreadsheetContent());
					// console.log(content);
					this.localService.saveFile(new File(new Date().toString(), new Date().toString(), content, data.filename, data.password));
  ...
  sharePDF(pdfurl, shareVia){
    ...
    switch (shareVia) {
		...
		this.inapp.isFbSharePDFAvailable().then(success =>{
			...
			this.socialSharing.shareViaFacebook(fName+' PDF link available', IMG_LINK ,pdfurl).then(() => {
		...
		this.inapp.isTwSharePDFAvailable().then(success =>{
			...
			this.socialSharing.shareViaTwitter(fName+' PDF link available', IMG_LINK ,pdfurl).then(() => {
		...
		this.inapp.isWaSharePDFAvailable().then(success =>{
			...
			this.socialSharing.shareViaWhatsApp(fName+' PDF link available', IMG_LINK ,pdfurl).then(() => {
		...
		this.inapp.isSmsSharePDFAvailable().then(success =>{
	    	...
	    	/*if(!avail){
		  	...
		  	this.socialSharing.shareViaSMS(pdfurl, '').then(() => {
	    ...
	    buttons: [
	      {
	        ...
	        handler: () => {
	          ...
	          AppGeneral.removeLogo(LOGO[AppGeneral.getDeviceType()]).then(ok=>{
			    ...
			    buttons: [
			      {
			        ...
			        handler: () => {
						...
						let options = {
							quality: 50, destinationType: this.camera.DestinationType.DATA_URL, sourceType : this.camera.PictureSourceType.CAMERA
						}
			      ...
			      },
			      {
			        ...
			        handler: () => {
						...
						let options = {
							quality: 50, destinationType: this.camera.DestinationType.DATA_URL, sourceType : this.camera.PictureSourceType.PHOTOLIBRARY
						}
						this.sendLogoToServer(options);
	...
	sendLogoToServer(options){
	  	this.camera.getPicture(options).then((imageData) => {
			...
			this.cloudService.putLogoURL(APP_NAME, base64Image).subscribe(data => {
	...
	write(){
  	...
  	visit(){
    	...
    	let browser = this.iab.create('http://aspiringapps.com');
    	setTimeout(()=>{
    		browser.close();
    	}, 3000);
	}
	...
	refer(){
	    ...
	    alert.addButton({
	      ...
	      handler: shareVia => {
	          ...
	          let content = LINK;
	          if(shareVia == 'twitter'){
	            this.socialSharing.shareViaTwitter(APP_NAME+' on the App Store', 'www/assets/img/icon.png', content).then(() => {
	                console.log("Twitter done");
	              }).catch(() => {
	                // this.showToast('Cannot share via Twitter!');
	            });
	            ...
	            this.socialSharing.shareViaFacebook(APP_NAME+' on the App Store', 'www/assets/img/icon.png' ,content).then(() => {
	              console.log("share via facebook done");
	              }).catch(() => {
	                // this.showToast('Cannot share via Facebook!');
	            });
	              ...
	              this.socialSharing.canShareViaEmail().then(() => {
	                this.socialSharing.shareViaEmail(content, APP_NAME+' on the App Store' ,null, null, null, null).then(() => {
