...
import { PDF_10, PDF_25, PDF_50, PDF_100, SPE_10, SPE_500, SPE_1000, SAVE_PDF , FB_10, TW_10, WA_10, SMS_10 } from './app-data';
	...
	isSavePrintEmailAvailable(){
		return this.getInappItems().then(products =>{
		    ...
		    if(products[9].Purchase == "Yes"){
	            ...
	            let units = parseInt(products[10].Own) - parseInt(products[10].Consumed);
	            if(units > 0) return true;
		    }
		    else if(products[11].Purchase == "Yes"){
	            let units = parseInt(products[11].Own) - parseInt(products[11].Consumed);
	            if(units > 0) return true;
			}
	...
	updateSavePrintEmail(){
		return Observable.create(observer => {
			...
			this.getInappItems().then(products =>{
				// console.log(products);
				let itemConsumed=9;
			    if(products[9].Purchase == "Yes" && products[9].Consumed <= products[9].Own){ itemConsumed = 9; }
			    else if(products[10].Purchase == "Yes" && products[10].Consumed <= products[10].Own){ itemConsumed = 10; }
			    else if(products[11].Purchase == "Yes" && products[11].Consumed <= products[11].Own){ itemConsumed = 11; }
			    else{
			...
			this.getInappItems().then(products =>{
				switch(id) {
		            ...
		            case SPE_10:
				            ...
				            products[9].Own=10 + left_10 + (products[9].Own - products[9].Consumed);
		            ...
		            case SPE_500:
				            ...
				            products[10].Own=500 + left_500 + (products[10].Own - products[10].Consumed);
		            ...
		            case SPE_1000:
				            ...
				            products[11].Own=1000 + left_1000 + (products[11].Own - products[11].Consumed);
